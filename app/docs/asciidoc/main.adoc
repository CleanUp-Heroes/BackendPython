...
.**Objectif** :
Renforcer la fiabilité des participations aux défis en analysant automatiquement les preuves visuelles à l’aide d’une API de reconnaissance d’objets (ex : Google Vision API). L’objectif est de s’assurer que la quantité déclarée correspond réellement au contenu visible sur la photo.
...
* Le backend compare :
  ...
  * Le **nombre ou type d’objets détectés** sur la photo.
  * La **quantité indiquée** dans le formulaire.
...
.**But principal** :
Renforcer la **vérifiabilité**, **limiter les abus**, et **fiabiliser les statistiques** tout en offrant à l’utilisateur une **seconde chance** en cas d’erreur.
